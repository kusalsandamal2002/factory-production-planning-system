    ...
    * Same plan-date + same stock payload is ignored. A changed payload is kept as
    ...
    def bootstrap_from_committed_imports(
        ...
        *,
        ...
        try:
            live_source = OperationalSourceService.latest(session)
            live_import_run_id = live_source.import_run_id
        except Exception:
            live_import_run_id = None
        ...
        for candidate in reversed(list(candidates)):
            ...
            result = cls.capture_import(
                ...
                import_mode=(
                    "LIVE"
                    if live_import_run_id is not None
                    and int(candidate["id"]) == int(live_import_run_id)
                    else "HISTORICAL"
                ),
            )
            files += 1
