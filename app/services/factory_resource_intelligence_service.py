...
class FactoryResourceIntelligenceService:
    ...
    def seed_technical_registry(cls, session) -> dict[str, int]:
        ...
        latest = source.plan_date or date.today()
    ...
    def _latest_live_context(cls, session) -> dict[str, Any]:
        ...
        return cls._safe_row(
            session,
            """
            SELECT import_run_id, MAX(plan_date) AS plan_date, MAX(source_workbook) AS workbook_name
            FROM mpps_fi_plan_allocations
            WHERE plan_date < DATE '2060-01-01'
            GROUP BY import_run_id
            ORDER BY MAX(plan_date) DESC, import_run_id DESC
            LIMIT 1
            """,
        )
