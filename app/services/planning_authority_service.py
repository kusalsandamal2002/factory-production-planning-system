...
class PlanningAuthorityService:
    ...
    def _stock_snapshot(cls, connection) -> tuple[dict[str, int], dict[str, Any]]:
        ...
        if cls._table_exists(connection, "mpps_current_stock_snapshots"):
            latest = None
            live_source = OperationalSourceService.latest(connection)
            if live_source.import_run_id:
                latest = connection.execute(
                    ...
                    {"run_id": int(live_source.import_run_id)},
                ).mappings().first()
            if latest is None:
                latest = connection.execute(
                    text(
                        ...
                        SELECT import_run_id, MAX(plan_date) AS plan_date
                        FROM mpps_current_stock_snapshots
                        WHERE plan_date IS NULL OR plan_date < DATE '2060-01-01'
                        GROUP BY import_run_id
                            ...
                            ORDER BY
                                CASE
                                    ...
                                    WHEN s.target_date IS NOT NULL
                                     AND s.target_date < DATE '2060-01-01'
                                ...
                                END,
                                CASE
                                    WHEN NOT COALESCE(s.target_date_is_manual,FALSE)
                                     AND s.target_date >= DATE '2060-01-01'
                                    THEN NULL
                                    ELSE s.target_date
                                END ASC NULLS LAST,
