...
# MPPS R7463 HOTFIX ML SNAPSHOT READ ONLY
def _mpps_r7463_snapshot_compat(cls):
    def as_int(v):
        try:
    ...
    def rows(connection, table):
        if not cls._table_exists(connection, table):
            return []
        return [dict(r) for r in connection.execute(text(f'SELECT * FROM "{table}"')).mappings().all()]

    with engine.connect() as connection:
        history_rows, history_days = cls._history_span(connection)
        registry_rows = rows(connection, "mpps_ml_model_registry_v2")
        version_rows = rows(connection, "mpps_ml_model_versions_v2")
        legacy_rows = rows(connection, "mpps_tyre_ml_registry")

    registry = {}
    for row in registry_rows:
        key = str(row.get("model_key") or row.get("module_key") or "").strip()
        if key:
            registry[key] = row
    ...
    for row in legacy_rows:
        for value in (row.get("module_key"), row.get("module_name")):
            if value:
                legacy[cls._norm(value)] = row

    models = []
    for model_key, area, name in MODEL_CATALOG:
        reg = registry.get(model_key) or {}
        vr = versions.get(model_key) or []
        champs = [
            r for r in vr
            if str(r.get("status") or "").upper() == "CHAMPION"
            or bool(r.get("champion"))
            or bool(r.get("is_champion"))
        ]
        pool = champs or vr
        ver = max(pool, key=lambda r: (as_int(r.get("id")), str(r.get("created_at") or ""))) if pool else {}

        old = {}
        if not reg and not ver:
            target = cls._norm(name)
            for k, candidate in legacy.items():
        ...
        status = str(merged.get("status") or "REGISTERED").upper()
        if champs:
            status = "CHAMPION"

        training_rows = as_int(
            merged.get("training_rows") or merged.get("train_rows") or merged.get("row_count")
        )
        model_history_days = as_int(merged.get("history_days")) or history_days
        confidence = as_float(
            merged.get("confidence_score")
            if merged.get("confidence_score") is not None
            else merged.get("test_score")
        )
        readiness = confidence * 100.0 if 0 <= confidence <= 1.0 else confidence

        if not reg and not ver and not old:
            status = "NEEDS TRAINING" if history_days >= 30 else "NEEDS DATA"
        ...
        models.append({
            ...
            "history_days": model_history_days,
            "readiness_score": readiness,
            "model_version": str(merged.get("model_version") or merged.get("version") or "R7-UNTRAINED"),
            "last_trained_at": merged.get("last_trained_at") or merged.get("trained_at") or merged.get("created_at"),
            "last_data_update": merged.get("last_data_update") or merged.get("data_end") or merged.get("updated_at"),
        })

    ready_states = {"READY", "TRAINED", "ACTIVE", "CHAMPION"}
    learning_states = {"LEARNING", "REGISTERED", "NEEDS TRAINING"}
    ready = sum(str(r.get("status") or "").upper() in ready_states for r in models)
    learning = sum(str(r.get("status") or "").upper() in learning_states for r in models)
...
MLPlatformService.snapshot = classmethod(_mpps_r7463_snapshot_compat)
