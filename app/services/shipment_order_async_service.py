...
def load_previous_shipments(search: str = "", limit: int = 20) -> list[dict[str, Any]]:
    ...
    query = f"""
        SELECT
            ...
            COALESCE(
                CASE WHEN NOT COALESCE(s.target_date_is_manual,FALSE) AND s.target_date >= DATE '2060-01-01' THEN NULL ELSE s.target_date END,
                CASE WHEN s.plan_date >= DATE '2060-01-01' THEN NULL ELSE s.plan_date END,
                CASE WHEN s.manager_order_date >= DATE '2060-01-01' THEN NULL ELSE s.manager_order_date END,
                s.factory_out_date
            ) AS target_date,
            COALESCE(
                s.factory_out_date,
                MAX(i.item_receive_date)
                    + GREATEST(0, COALESCE(s.dispatch_buffer_days, 0)),
...
def get_unallocated_stock(sap_code: str) -> int:
    ...
    with engine.connect() as connection:
        ...
        latest_run_id = None
        if current_table:
            latest_run_id = OperationalSourceService.latest(
                connection
            ).import_run_id
        ...
        if has_current:
            available = int(
                connection.execute(
                    ...
                    {"sap_code": canonical, "run_id": int(latest_run_id)},
                ).scalar()
                or 0
            )
        if not has_current:
