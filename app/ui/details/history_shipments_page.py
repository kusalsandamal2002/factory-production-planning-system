...
class HistoryShipmentsPage(QWidget):
    SHIPPED_STATUSES = (
        "SHIPPED",
        "DISPATCHED",
        "DELIVERED",
    ...
    def __init__(self, on_back=None):
        ...
        self.rows: list[HistoryShipmentRow] = []

        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText(
            "Search history order no, customer, status, priority or note..."
        )
        self.search_input.setMinimumHeight(42)
        self.search_input.textChanged.connect(lambda _text: self.filter_table())

        self.type_filter = QComboBox()
        self.type_filter.setMinimumHeight(42)
        self.type_filter.addItems(["All History", "Shipped", "Cancelled"])
        self.type_filter.currentTextChanged.connect(lambda _text: self.filter_table())
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Shipment History")
        title.setObjectName("SectionTitle")

        hint = QLabel(
            "Completed shipped records and cancelled shipments are stored here for history review."
        )
    ...
    def refresh(self) -> None:
            ...
            with get_session() as session:
                orders = list(
                    session.scalars(
                        ...
                        .where(func.upper(Order.status).in_(self.HISTORY_STATUSES))
        ...
        self.rows = [
            HistoryShipmentRow(
                ...
                history_type=self._history_type(str(order.status or "-").upper()),
    ...
    def _refresh_table(self) -> None:
        ...
        for row_number, row_data in enumerate(self.rows):
            ...
            for col, value in enumerate(values):
                ...
                if col == 2:
                    self._apply_history_type_style(item, row_data.history_type)
    ...
    def filter_table(self) -> None:
        ...
        type_value = self.type_filter.currentText().strip().upper()
        visible_count = 0
        ...
        for row in range(self.table.rowCount()):
            ...
            history_type = self.table.item(row, 2).text().upper() if self.table.item(row, 2) else ""

            search_ok = not search_value or search_value in row_text
            type_ok = type_value == "ALL HISTORY" or history_type == type_value.upper()

            hide_row = not (search_ok and type_ok)
    ...
    def _history_type(self, status: str) -> str:
        status_value = (status or "").upper()

        if status_value in self.CANCELLED_STATUSES:
            return "Cancelled"
    ...
    def _apply_history_type_style(self, item: QTableWidgetItem, history_type: str) -> None:
        if history_type.upper() == "CANCELLED":
