            ...
            rows = self._query(
                ...
                SELECT
                    ROW_NUMBER() OVER (
                        ORDER BY
                            CASE
                                WHEN target_date_is_manual THEN 0
                                WHEN target_date IS NOT NULL AND target_date < DATE '2060-01-01' THEN 0
                                ELSE 1
                            END,
                            COALESCE(
                                CASE WHEN NOT target_date_is_manual AND target_date >= DATE '2060-01-01' THEN NULL ELSE target_date END,
                    ...
                    CASE
                        ...
                        OR LOWER(COALESCE(planning_status, ''))
                            = 'review required'
                        OR (
                            NOT COALESCE(target_date_is_manual, FALSE)
                    ...
                    END AS target_date,
                    CASE
                        WHEN NOT COALESCE(target_date_is_manual, FALSE)
                         AND target_date >= DATE '2060-01-01'
