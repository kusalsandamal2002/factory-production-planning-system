...
class _DictModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.ToolTipRole:
            ...
            if key in {"item_status", "schedule_reason"}:
                reason = str(
                    row.get("schedule_reason")
                    or row.get("factory_out_reason")
                    or ""
                ).strip()
                if reason:
                    return reason
        ...
        if role == Qt.ItemDataRole.ForegroundRole:
            ...
            if key == "item_status":
                ...
                if "BLOCKED" in status:
                    return QColor("#b91c1c")
                if status in {"STOCK READY", "COMPLETED", "STOCK COVERED"}:
                    return QColor("#047857")
                if "PRODUCTION" in status or "PLANNED" in status:
                    return QColor("#1d4ed8")
...
DETAIL_COLUMNS = [
    ...
    ("Block / Planning Reason", "schedule_reason", _reason_text),
]
...
class ShipmentDetailsProPage(QWidget):
    ...
    def _detail_loaded(self, payload: dict[str, Any]) -> None:
        ...
        planning_note = str(shipment.get("planning_note") or "").strip()
        status_text = (
            f"Status: {operational}  •  Lifecycle: {lifecycle}  •  "
