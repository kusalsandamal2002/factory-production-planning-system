...
class OverdueOrdersPage(QWidget):
    ...
    def __init__(self, on_back=None):
        ...
        self.search_input.setPlaceholderText(
            "Search overdue order no, customer, status, priority or note..."
        )
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Overdue Production Orders")
        title.setObjectName("SectionTitle")

        hint = QLabel(
            "Only orders with confirmed receive dates earlier than today and not completed are shown here."
        )
            ...
            with get_session() as session:
                orders = list(
                    session.scalars(
                        ...
                        .order_by(Order.manager_confirmed_receive_date.asc(), Order.id.desc())
                    )
                )
    ...
    def _refresh_table(self) -> None:
        ...
        for row_number, row_data in enumerate(self.rows):
            ...
            for col, value in enumerate(values):
                ...
                if col == 6:
                    self._apply_days_late_style(item, row_data.days_late)
    ...
    def _apply_days_late_style(self, item: QTableWidgetItem, days_late: int) -> None:
        if days_late >= 7:
            item.setForeground(QColor("#991b1b"))
            item.setBackground(QColor("#fee2e2"))
        elif days_late >= 1:
