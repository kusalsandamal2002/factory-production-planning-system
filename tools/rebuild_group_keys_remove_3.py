...
EXPORT_FILE = Path("exports/tyre_group_key_audit_simplified.xlsx")
...
with engine.begin() as conn:
    conn.execute(text(f"""
        CREATE TABLE IF NOT EXISTS tyre_process_groups_backup_remove3_{TIMESTAMP}
        AS SELECT * FROM tyre_process_groups
    """))

    conn.execute(text(f"""
        CREATE TABLE IF NOT EXISTS tyre_process_group_items_backup_remove3_{TIMESTAMP}
    ...
    for row in rows:
        ...
        for field_name in ["tyre_size", "rim_width", "tread_pattern", "layer", "color"]:
...
ws_summary.append(["Key Method", "SIZE_RIM_TREAD_LAYER_COLOR_V3"])
ws_summary.append(["SAP Codes", len(item_rows)])
ws_summary.append(["Group Keys", len(groups)])
ws_summary.append(["Removed From Key", "Family, Construction, STD Flag"])
...
ws_groups.column_dimensions["A"].width = 45
ws_items.column_dimensions["A"].width = 45
ws_items.column_dimensions["H"].width = 55
ws_audit.column_dimensions["B"].width = 45
ws_audit.column_dimensions["D"].width = 55

wb.save(EXPORT_FILE)

print("Simplified tyre group key rebuild completed.")
print("SAP Codes:", len(item_rows))
print("Group Keys:", len(groups))
print("Audit Rows:", len(audit_rows))
print("Removed columns: Family, Construction, STD Flag")
