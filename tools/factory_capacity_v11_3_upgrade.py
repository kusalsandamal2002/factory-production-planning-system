...
def run(max_files: int = 0) -> dict[str, int]:
    """Resumable one-time V11.3 history upgrade.
    ...
    print("[ 92%] Rebuilding unique LINE/CAVITY/MOLD/CASING memory...")
    with get_session() as session:
        service.ensure_schema(session)
        service.seed_technical_registry(session)
    ...
    print("[ 96%] Refreshing backend capacity/ML profiles (UI remains separate from training)...")
    with get_session() as session:
        service.ensure_schema(session)
        try:
            training = service.train_profiles(session)
            print(
                "ML: profiles={profiles} device={device} advanced={advanced}".format(
                    profiles=training.get("fi_capacity_profiles_trained", 0),
                    device=training.get("fi_training_device", "cpu"),
                    advanced=training.get("fi_advanced_ml_family", "ROBUST_ENSEMBLE"),
                )
            )
        except Exception as exc:
            # Resource UI/data migration must still succeed when an optional ML
            # package/GPU driver is unavailable. The robust engine trains on the next
            # Intelligent Excel Import as well.
            print(f"[WARN] Optional ML refresh skipped: {exc}")

    print("[100%] Finalizing Factory Resource & Capacity state...")
    with get_session() as session:
        service.ensure_schema(session)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="MPPS Factory Resource & Capacity V11.3 one-time migration/backfill")
    ...
    print("V11.3 migration complete:", result)
